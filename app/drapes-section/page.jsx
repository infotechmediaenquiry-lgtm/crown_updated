...
export default function DrapesSectionPage() {
  const [currentPage, setCurrentPage] = useState(1);
  const productsPerPage = 12;
  
  const sectionSlug = 'drapes-section';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>›</span>
            <span className="text-gray-900 font-medium">Drapes Section</span>
          </div>
        </div>

        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Drapes Section</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
          {currentProducts.map((product) => (
            <Link
              key={product.id}
              href={`/drapes-section/${product.id}`}
              className="group"
            >
